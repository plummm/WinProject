...
using ManagerAssistance.Master.Remote;
using System.IO;
...
namespace ManagerAssistance.Master
{
    ...
    /// </summary>
    public partial class FileManagerWindow : ClientWindow
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public FileManagerWindow(Client client) : base(client)
        {
            InitializeComponent();
        ...
        private void UploadFile(string uri)
        {
            ...
            AddCommand(CommandFactory.CreateCommand<UploadProgressMessage>(
                ...
                progress =>
                {
                    FireClientEvent(ClientEventType.FileUploaded, fileName);
        ...
        private void DownloadFile(string name)
        {
            ...
            AddCommand(CommandFactory.CreateCommand<DownloadedFilePartMessage>(
                ...
                download => // continue
                {
                    try
                    {
                        FileHelper.WriteDownloadedPart(Client.OutputDirectory, download.Path,
                ...
                download => // complete
                {
                    try
                    {
                        FileHelper.WriteDownloadedPart(Client.OutputDirectory, download.Path, download.CurrentPart, download.RawPart);
                        //TotalTransaction.Content = $"Successful transactions: {++m_sucessfulTransactions}/{m_totalTransactions}";
                        FireClientEvent(ClientEventType.FileDownloaded, Path.GetFileName(download.Path));
                        FinishCurrentCommand();
                    }
                    catch (Exception e)
                    {
                ...
                switch ((ClientEventType)ev.EventType)
                {
                    case ClientEventType.IncommingMessage:
                    {
                        ev.Data.Match()
                            ...
                            {
                                // will be received when the client download the file
                                UpdateCommand(m, command =>
                                {
